...
window.set_hexpand(true);
    ...
    css_provider.load_from_path("src/components/styles/app.css").expect("Failed to load CSS stylesheet");

        let main_container = gtk::Grid::new();
        main_container.get_style_context().map(|c| c.add_class("main_container"));
